...
describe('UserAccount', () => {
  ...
  it('renders ActivityIndicator is loading is true', () => {
    const { queryByTestId } = render(<UserAccount data={mockData} navigate={mockProps.navigation.navigate} loading={true}/>);
  ...
  it('renders List is loading is false', async() => {
    ...
    modifiedApiMock.viewer.offers = mockData.data.viewer.offers.filter((item, index) => index < 2);

    const { queryByTestId } = render(<UserAccount data={mockData.data} navigate={mockProps.navigation.navigate} loading={false}/>);
  ...
  it('renders error text when fetching data gone wrong', async() => {
    const { queryByTestId } = render(<UserAccount data={mockData.data} navigate={mockProps.navigation.navigate} loading={false} error={true}/>);
  ...
  it('render correct currency format on user balance', async() => {
    const wrapper = render(<UserAccount data={mockData.data} navigate={mockProps.navigation.navigate} loading={false} error={true}/>);
